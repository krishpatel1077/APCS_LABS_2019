...
class TJTwitter2 
{
   ...
   {
      //your code goes here      
      for (TJ_Status2 str : statuses)
      {
         String[] array = str.getText().split(" ");
         for (int i = 0; i < array.length; i++){
            terms.add(removePunctuation(array[i]));
         }
      }
   }
    ...
    */
   @SuppressWarnings("unchecked")
   public void removeCommonEnglishWords(File f)
   {
      //your code goes here
      try{  
         Scanner sc = new Scanner(f);
         String[] arr = new String[91];
         int index = 0;
         ...
         for (int i = 0; i < terms.size(); i++){
            for (int x = 0; x < arr.length; x++){
               if (terms.get(i).equalsIgnoreCase(arr[x])){
                  terms.remove(i);
               }
            }
         }
   ...
   {
      //your code goes here      
      ListIterator<String> l = terms.listIterator();
      while (l.hasNext()){
         if (l.next() == null)
            l.remove();
      }
      String[] str = new String[terms.size()];
      for (int i = 0; i < str.length; i++)
         str[i] = terms.get(i);
      sort(str);
      terms = new ArrayList<String> (Arrays.asList(str));
   }
